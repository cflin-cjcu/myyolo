...
def build_target(preds, targets, cfg, device):
    ...
    anchors = torch.from_numpy(anchors.reshape(
        len(preds) // 3, anchor_num, 2)).to(device)

    gain = torch.ones(7, device=device)

    at = torch.arange(anchor_num, device=device).float().view(
        anchor_num, 1).repeat(1, label_num)
    ...
    off = torch.tensor([[0, 0],
                        ...
                        ], device=device, dtype=torch.float32).float() * g  # offsets
...
def compute_loss(preds, targets, cfg, device):
    ...
    for i, pred in enumerate(preds):
        ...
        if i % 3 == 0:
            pred = pred.reshape(
                pred.shape[0], cfg["anchor_num"], -1, pred.shape[2], pred.shape[3])
            ...
            if len(indices):
                ...
                if nb:
                    ...
                    pwh = (ps[:, 2:4].sigmoid() * 2) ** 2 * \
                        anchors[layer_index[i]]
                    pbox = torch.cat((pxy, pwh), 1)  # predicted box
                    # ciou(prediction, target)
                    ciou = bbox_iou(
                        pbox.t(), tbox[layer_index[i]], x1y1x2y2=False, CIoU=True)
                    lbox += (1.0 - ciou).mean()
        ...
        # 计算obj分支loss
            ...
            lobj += BCEobj(pred[..., 0], tobj) * \
                balance[layer_index[i]]  # obj loss

        # 计算cls分支loss
        elif i % 3 == 2:
            pred = pred.reshape(
                pred.shape[0], 1, -1, pred.shape[2], pred.shape[3])
            pred = pred.permute(0, 1, 3, 4, 2)
            ...
            if len(indices):
                ...
                if nb:
                    ...
                    if ps.size(1) > 1:
                        ...
                        lcls += BCEcls(ps[:, :], tcls[layer_index[i]]
                                       ) / cfg["classes"]  # BCE
        else:
            print("error")
            raise
